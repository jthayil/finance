...
class InvoiceSerializer(serializers.ModelSerializer):
    ...
    def get_qty(self, obj):
        return InvoiceItems.objects.filter(invoice_id=obj.id).aggregate(total=Sum('qty'))['total'] or 0

    def get_taxable_amt(self, obj):
        return sum(item.taxable_amount() for item in InvoiceItems.objects.filter(invoice_id=obj.id))

    def get_tax(self, obj):
        return sum(item.tax_amt() for item in InvoiceItems.objects.filter(invoice_id=obj.id))

    def get_total_amt(self, obj):
        amt = sum(item.total_amount() for item in InvoiceItems.objects.filter(invoice_id=obj.id)) + self.get_tax(obj)
        return amt.__round__(2)

    class Meta:
        model = Invoices
        fields = ['id', 'inserted_On', 'buyer', 'invoice_no', 'qty', 'taxable_amt', 'tax', 'total_amt', 'status']
